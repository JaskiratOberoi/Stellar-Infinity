...
| G16 | Downloading marks the sample Printed (6→8, 7→9) | `ChangeSampleStatus` | **DONE (2026-09-09)** | `usp_inf_report_mark_printed` (script 143): 6→8 / 7→9 only, under UPDLOCK, one `inf_result_audit` row (action `status`, old→new, actor, IP, agent, channel in `reason`). Called from the single and bulk PDF routes AFTER the bytes exist (rendered or cached) — a failed render never marks. QR copy and viewer do not mark, as in the legacy. A printed sample leaves 8/9 only via the audited reopen (52); the save (51) refuses 7/8/9. See port-decisions. |
| G17 | Audit of who downloaded which report | `GetUserLog(…, "Printed …")` → `sp_user_activity_log` | **DONE (2026-09-09)** | `report.pdf` per report delivered (role, paper, cache, status flip), on the single route and — new — per report inside a bundle; `report.pdf_bulk` names the batch's SIDs; `report.pdf_failed` for a render that produced nothing; `report.graph`, `report.smart_viewed`, `report.public_viewed` / `report.public_pdf` (QR, IP only) close the remaining egress gaps; the renderer's own data fetch is `report.rendered`, no longer a phantom `report.viewed`. |
