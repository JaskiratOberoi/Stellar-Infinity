...
export const DEFAULTS = Object.freeze({
  /** Accreditation certificate number, printed beneath the symbol. Empty: symbol alone. */
  cert: 'MC-2547',
  /** Where the foot symbol goes: header-/footer- × left/center/right, or off. Beside the QR by default. */
  mark: 'footer-center',
  ...
  rowSize: 4,
  /** Where the QR goes, same anchors as `mark`. Items sharing an anchor sit side by side, symbol first. */
  qr: 'footer-center',
  qrPages: 'all',
...
const ANCHORS = ['header-left', 'header-center', 'header-right', 'footer-left', 'footer-center', 'footer-right', 'off'];
 ...
 * @param {{ qrUrl?: string|null, rows?: Array }} qrUrl: the URL the QR should carry (null draws no QR);
 *        rows: parameter-name positions to mark, see normalizeRows
 * @returns {{ bytes: Uint8Array, pages: number, options: object, stamped: { mark: boolean, qr: boolean, rows: number } }}
 */
export async function seal(input, rawOptions = {}, { qrUrl = null, rows = [] } = {}) {
  ...
  pages.forEach((page, i) => {
    ...
    if (markImg && opts.mark !== 'off' && onPage(opts.markPages, i, n)) put(opts.mark, symbolItem(markImg, opts, bold));
    if (qrImg && onPage(opts.qrPages, i, n)) put(opts.qr, qrItem(qrImg, opts, font));
  ...
  return {
    ...
    pages: n,
    options: opts,
    stamped: { mark: Boolean(markImg) && opts.mark !== 'off', qr: Boolean(qrImg), rows: rowMarks.length },
  };
}
