/*
 * 146_alter_telo_order_package_expansion.sql
 *
 * MODIFIES TWO TELO-OWNED PROCEDURES. dbo.usp_telo_create_order and
 * dbo.usp_telo_add_sids are shared with Telo; as script 108 did for the
 * first of them, this carries the FULL bodies — Telo's 60_ and 65_ source
 * files, which matched the deployed definitions on 2026-09-19 — plus one
 * change applied identically to both, so the objects stay reproducible from
 * source. Telo's files carry the same change, so a Telo deploy:sp cannot
 * undo it. The pre-change definitions were saved before this first ran.
 *
 * THE CHANGE — a package expands by its definition, and the tube reads as
 * the LIS writes it:
 *
 *   • The child profiles and tests of a master profile are no longer
 *     filtered on IsActive. HEALTH SCREEN 3 and nine other live packages
 *     contain Thyroid Profile II, which is flagged inactive; the filter
 *     silently dropped it, and with it FT3, FT4 and TSH, from every package
 *     booked through Infinity or Telo — six such reports went out before
 *     this was found. The LIS reads a package's members with no active
 *     filter (MedCis.Business/Pcc/WorkOrder.cs), so a legacy booking of the
 *     same package carries the profile. Direct test and profile lines keep
 *     their active check.
 *   • The tube's testcodes/testtypes/testnames CSVs are written in the LIS's
 *     order — a package's profiles first, then its tests, each in the package
 *     definition's row order and under the definition's own member names —
 *     and the package name is tagged onto the package's last profile and
 *     last test as the LIS does: "…&nbsp;<i><b>[HEALTH SCREEN 3]</b></i>".
 *     Without the tag the legacy Sample Worksheet, which shows only that
 *     string, listed an Infinity-booked package as loose tests.
 *
 * Everything else in both bodies is byte-for-byte the deployed text.
 */
...
BEGIN
    ...
    item_resolution AS (
        ...
        JOIN dbo.tbl_med_test_master t ON t.id = pp.testid AND (t.IsActive = 1 OR s.fromMaster = 1)
        WHERE s.isProfile = 1
    ),
    profile_span AS (
