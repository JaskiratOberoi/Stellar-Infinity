...
test('seals every page, keeps the page count, embeds the two images once', async () => {
  ...
  assert.deepEqual(stamped, { mark: true, qr: true, rows: 0 });
...
test('first-page-only and off are honoured', async () => {
  ...
  assert.deepEqual(stamped, { mark: true, qr: false, rows: 0 });
  const out = await PDFDocument.load(bytes);
...
test('row marks: drawn on their page, shared image, tolerant of junk', async () => {
  ...
  assert.deepEqual(stamped, { mark: false, qr: false, rows: 3 });
  const out = await PDFDocument.load(bytes);
...
test('the usual defaults: symbol and qr share the bottom centre, certificate MC-2547', () => {
  const o = normalizeOptions({});
  assert.equal(o.cert, 'MC-2547');
  assert.equal(o.mark, 'footer-center');
  assert.equal(o.qr, 'footer-center');
